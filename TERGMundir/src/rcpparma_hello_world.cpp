                ...
                for (int t=0; t<T_data; t++){
                    for(int j = i+1; j < N; j++){
                        for(int l = 0; l < K; l++){
                            ...
                            t1+=(gamma(j,l)/(2*gamma(i,k)))*((network(i,j,t)*(theta(k)+theta(l)))-log(1+exp_val));
                        }
                    }
                }
                quad_lin_coeff(i,k,0)=t1-(T_data/gamma(i,k));
                quad_lin_coeff(i,k,1)=T_data*(log(alpha(k))-log(gamma(i,k))+1);
            }
        } else if(i==(N-1)){
            for(int k = 0; k < K; k++){
                quad_lin_coeff(i,k,0)=-(T_data/gamma((N-1),k));
                quad_lin_coeff(i,k,1)=T_data*(log(alpha(k))-log(gamma((N-1),k))+1);
            }
        }
    }
    ...
    for (int t=0; t<T_data; t++){
        ...
        for(int i = 0; i < (N-1); i++){
            for(int j = i+1; j < N; j++){
                ...
                for(int k = 0; k < K; k++){
                    for(int l = 0; l < K; l++){
                        grad_matsub(k,l)=gamma(i,k)*gamma(j,l)*(network(i,j,t)-(exp_val_mat(k,l)/(1+exp_val_mat(k,l))));
                    }
                }
    ...
    for (int t=0; t<T_data; t++){
        for(int i = 0; i < (N-1); i++){
            for(int j = i+1; j < N; j++){
                for(int k = 0; k < K; k++){
                    for(int l = 0; l < K; l++){
                        t1+=(gamma(i,k)*gamma(j,l)*(network(i,j,t)*(theta(k)+theta(l))-log(1+exp_val_mat(k,l))));
                    }
                }
            }
        }
...
float grad_TERGM_undir_K1(float theta, cube network, int N, int T_data){
    ...
    for (int t=0; t<T_data; t++){
        ...
        for(int i = 0; i < (N-1); i++){
            for(int j = i+1; j < N; j++){
                grad_val+=(network(i,j,t)-exp_val_ratio);
            }
        }
        grad_vector(t)=2*grad_val;
    }
    float grad_val_final=accu(grad_vector);
...
float hess_TERGM_undir_K1(float theta, int N, int T_data){
    ...
    for (int t=0; t<T_data; t++){
        ...
        hess_vec(t)=4*hess_val;
    }
    float hess_val_final=accu(hess_vec);
