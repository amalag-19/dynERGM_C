                ...
                for (int t=0; t<(T_data-1); t++){
                    for(int j = i+1; j < N; j++){
                        for(int l = 0; l < K; l++){
                            ...
                            t1+=(gamma(j,l)/(2*gamma(i,k)))*((Stab_stat*(theta(k)+theta(l)))-log(1+exp_val));
                        }
                    }
                }
        ...
        for(int i = 0; i < (N-1); i++){
            for(int j = i+1; j < N; j++){
                ...
                for(int k = 0; k < K; k++){
                    for(int l = 0; l < K; l++){
                        int Stab_stat=(network(i,j,t+1)*network(i,j,t))+((1-network(i,j,t+1))*(1-network(i,j,t)));
                        grad_matsub(k,l)=gamma(i,k)*gamma(j,l)*(Stab_stat-(exp_val_mat(k,l)/(1+exp_val_mat(k,l))));
                    }
                }
        ...
        for(int i = 0; i < (N-1); i++){
            for(int j = i+1; j < N; j++){
                for(int k = 0; k < K; k++){
                    for(int l = 0; l < K; l++){
                        int Stab_stat=(network(i,j,t+1)*network(i,j,t))+((1-network(i,j,t+1))*(1-network(i,j,t)));
                        t1+=(gamma(i,k)*gamma(j,l)*(Stab_stat*(theta(k)+theta(l))-log(1+exp_val_mat(k,l))));
                    }
                }
            }
        }
